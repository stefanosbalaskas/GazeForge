...
def fit_correlated_location_scale(
    ...
    *,
    ...
    if _random_effect_sd_boundary_reached(tau_location, tau_scale):
        raise RuntimeError(
            "Correlated location-scale random-effect standard deviation reached an "
...
def _validate_result_identity(result: CorrelatedLocationScaleResult) -> dict[str, Any]:
    ...
    if not np.isfinite(random_effect_sds).all() or np.any(random_effect_sds <= 0.0):
        raise SchemaError(
            "Correlated location-scale random-effect standard deviations must be "
            "finite and positive."
        )
    if _random_effect_sd_boundary_reached(result.tau_location, result.tau_scale):
        raise SchemaError(
            "Correlated location-scale random-effect standard deviation is at an "
