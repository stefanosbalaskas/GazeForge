...
def _review_facts(row: pd.Series) -> tuple[str, str]:
    review_fingerprint = _text(row.get("scientific_review_fingerprint_sha256", ""))
...
def render_verified_suite_cards(dashboard: BenchmarkDashboard) -> str:
    ...
    for _, row in dashboard.suite_table.iterrows():
        ...
        facts = "".join(
            ...
            for part in (
                ...
                _fact("Target rate (Hz)", row.get("target_sampling_rate_hz", "")),
                _fact("Model", row.get("model", "")),
                _fact("Reference stream", row.get("reference_stream_id", "")),
                review_facts,
            )
            if part
        )
        provenance = "".join(
            ...
            for part in (
                ...
                _fingerprint("Suite fingerprint", row.get("suite_fingerprint_sha256", "")),
                review_provenance,
            )
            if part
        )
        cards.append(
            ...
            f'<div class="gf-evidence-kicker">Verified suite · {_html(row.get("status", ""))}</div>'
    ...
    return '<div class="gf-evidence-grid" role="list">' + "".join(cards) + "</div>"
    ...
    for _, row in dashboard.table.iterrows():
        ...
        facts = "".join(
            ...
            for part in (
                ...
                _fact("Sampling origin", row.get("sampling_origin", "")),
                _fact("Sampling rate (Hz)", row.get("sampling_rates_hz", "")),
                _fact("Reference strength", row.get("reference_strength", "")),
                _fact("Model family", row.get("models", "")),
                _fact("Annotation origin", row.get("annotation_origin", "")),
                review_facts,
            )
            if part
        )
        provenance = "".join(
            part
            for part in (
                _fingerprint("Report fingerprint", row.get("report_fingerprint_sha256", "")),
                review_provenance,
            )
            if part
        )
...
def _insert_cards_before_table(markdown: str, heading: str, cards: str) -> str:
    ...
    if heading_index < 0:
        raise BenchmarkIntegrityError(
            f"Evidence card renderer could not find expected section heading: {heading.strip()}"
        )
    table_index = markdown.find("\n| ", heading_index)
    if table_index < 0:
        raise BenchmarkIntegrityError(
            f"Evidence card renderer could not find provenance table after: {heading.strip()}"
        )
    return markdown[:table_index] + "\n\n" + cards + "\n" + markdown[table_index:]


def render_benchmark_dashboard_with_cards(dashboard: BenchmarkDashboard) -> str:
    """Add readable cards to the canonical dashboard without replacing its audit tables."""
