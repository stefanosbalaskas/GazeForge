...
Stop before fitting. Open the [Grouping, repeated measures & pseudoreplication clinic](grouping-repeated-measures.md). Preserve participant/trial/stimulus/AOI or event identities, distinguish nested from crossed design structure, and keep descriptive aggregation separate from inferential input. GazeForge does not automatically choose random effects, fixed effects, covariance structures, GEE, clustering corrections, or another estimator.
