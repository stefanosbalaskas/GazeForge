...
### :material-table-eye: I need to reconcile denominators, exposure, or censoring

Use this route before modelling when a zero may mean several different things, when
trial/AOI exposure differs, or when no-fixation latency must remain right-censored.

**Start here:** [Denominator, exposure & censoring clinic](denominator-exposure-censoring.md)  
...
analysis variants have been executed. Keep prespecified, exploratory, and deviation
analyses distinct; preserve failed/unevaluable variants; and compare only
same-estimand results directly.

**Start here:** [Sensitivity & robustness clinic](sensitivity-robustness-clinic.md)  
**Run it:** `python examples/15_worked_sensitivity_robustness_audit.py --output-dir worked-sensitivity-robustness-audit`  
**Continue to:** [Reporting clinic](reporting-clinic.md) · [Publication readiness](publication-readiness.md)
...
For a more complete research route, use the [Outcome & estimand preregistration clinic](estimand-preregistration.md) before modelling, then the [Grouping, repeated measures & pseudoreplication clinic](grouping-repeated-measures.md) before specialist fitting when rows repeat within participants/stimuli; the [Study lifecycle](study-lifecycle.md) ties every stage to a reviewable artifact and explicit claim boundary. The [Study-design templates](study-design-templates.md) make the corresponding records copy-ready.
