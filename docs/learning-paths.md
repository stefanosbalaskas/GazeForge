...
Use this route after the analysis/reporting artifacts are frozen and the external
reader needs to know which files support each statement, what can be rerun, which
inputs require authorized access, and which scientific limitations remain.

**Start here:** [Reviewer & replication handoff](reviewer-replication-handoff.md)  
**Run it:** `python examples/13_worked_reviewer_replication_bundle.py --output-dir worked-reviewer-replication-bundle`  
