...
| Audit uncertainty/multiplicity before reporting | diagnostically admissible fitted results + frozen estimand/family identity | [Uncertainty, multiplicity & inferential reporting clinic](inferential-reporting-audit.md) | `python examples/18_worked_inferential_reporting_audit.py --output-dir worked-inferential-reporting-audit` | result scale + interval identity + multiplicity family + interpretation gate | converged model ≠ correctly reported inference; raw p ≠ adjusted p | [Sensitivity & robustness clinic](sensitivity-robustness-clinic.md) |
