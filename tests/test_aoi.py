...
def test_callable_aoi_provider_materializes_detector_iterable():
    provider = CallableAOIProvider(
        lambda image, labels: (
            AOI(str(index), label, index, 0, index + 1, 1) for index, label in enumerate(labels)
        )
    )
...
def test_aoi_review_accept_and_reject_are_explicit_and_logged():
    ...
    assert log.astype(object).where(pd.notna(log), None).to_dict("records") == [
        {
            "aoi_id": "a1",
            "action": "accept",
            "before_label": "claim",
...
def test_aoi_review_replace_bounds_requires_all_coordinates_and_valid_geometry():
    ...
    incomplete = pd.DataFrame([{"aoi_id": "a1", "action": "replace_bounds", "xmin": 1}])
